                                                    ...
                                                    @else
                                                        ...
                                                        <div class="row">
                                                            <form action="{{ route('subscription.cart') }}" method="get">
                                                                ...
                                                                @foreach (App\SubscriptionCourse::where('subscription_duration','LIKE','%'.$subscription->duration.'%')->where('subscription_id', $subscription->subscription_id)->get() as $item)
                                                                    <input type="hidden" name="course_id[]" value="{{ $item->course_id }}">
                                                                @endforeach
                                                                @if (!expire($subscription->subscription_id))
                                                                <button type="submit" class="btn btn-primary">Renew Subscription</button>
                                                                @endif
                                                            </form>
                                                    @if (expire($subscription->subscription_id))
                                                        <a href="{{ route('my.subscription.package.course', [$subscription->subscription_package, $subscription->subscription_id]) }}" class="card-link btn btn-success ml-3">@translate(View Course)</a>
                                                    @endif
