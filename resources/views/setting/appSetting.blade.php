            ...
            <div class="card m-2">
                ...
                <div class="card-body">
                    <form method="post" action="{{route('app.update')}}" enctype="multipart/form-data">
                        ...
                        <hr class="py-2">
                        <label class="label">@translate(PAYME PRD URL) <a href="https://developer.help.paycom.uz/ru/poisk-klyucha-i-id-kassy-v-lichnom-kabinete/poisk-klyucha-parolya-ot-kassy" target="_blank"><i class="fa fa-info text-warning"></i></a></label>
                        <input type="hidden" name="types[]" value="PAYME_PRD_URL">
                        <input type="text" placeholder="@translate(Enter the data)" value="{{env('PAYME_PRD_URL')}}"
                               name="PAYME_PRD_URL"
                               class="form-control mb-2">

                        <label class="label">@translate(PAYME SUBSCRIBE KEY)</label>
                        <input type="hidden" name="types[]" value="PAYME_SUBSCRIBE_KEY">
                        <input type="text" placeholder="@translate(Enter the data)" value="{{env('PAYME_SUBSCRIBE_KEY')}}"
                               name="PAYME_SUBSCRIBE_KEY"
                               class="form-control mb-2">

                        <label class="label">@translate(PAYME SUBSCRIBE MERCHANT ID)</label>
                        <input type="hidden" name="types[]" value="PAYME_SUBSCRIBE_MERCHANT_ID">
                        <input type="text" placeholder="@translate(Enter the data)" value="{{env('PAYME_SUBSCRIBE_MERCHANT_ID')}}"
                               name="PAYME_SUBSCRIBE_MERCHANT_ID"
                               class="form-control mb-2">
                        <hr class="py-2">
                        {{-- Paytm --}}
