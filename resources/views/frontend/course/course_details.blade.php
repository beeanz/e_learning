                ...
                <div class="buy-course-btn mb-3 text-center">
                    @auth()
                        @if(\Illuminate\Support\Facades\Auth::user()->user_type == 'Student')
                            <form action="{{ route('subscription.cart') }}" id="form_phone" method="get">
                                @csrf
                                @foreach (json_decode($s_course->subscription->subscription_duration) as $item)
                                    <input type="hidden" value="{{ $item }}" name="subscription_package">
                                @endforeach
                                <input type="hidden" value="{{ $s_course->price }}" name="subscription_price">
                                <input type="hidden" value="{{ $s_course->subscription->subscription_id }}" name="subscription_id">

                                @foreach (App\SubscriptionCourse::where('subscription_id', $s_course->subscription->subscription_id)->get() as $item)
                                    <input type="hidden" name="course_id[]" value="{{ $item->course_id }}">
                                @endforeach
                                @auth
                                    @php
                                        $subscriptionEnrollment = App\SubscriptionEnrollment::where('user_id', Auth::user()->id)->where('subscription_id', $s_course->subscription->subscription_id);
                                    @endphp
                                    @if (!$subscriptionEnrollment->exists())
                                        <button type="submit" class="theme-btn btn-block">@translate(Subscribe now!)</button>
                                    @elseif(!expire($subscriptionEnrollment->first()->subscription_id))
                                        <button type="submit" class="theme-btn btn-block">@translate(Renew Subscription)</button>
                                    @else
                                        <button type="button" disabled class="theme-btn btn-block">@translate(Purchased)</button>
                                    @endif
                                @endauth
                            </form>
                        @else
                            <a href="{{route('login')}}" class="theme-btn w-100 mb-3">@translate(Subscribe now!)</a>
                        @endif
                    @endauth
                </div>
                                        ...
                                        <div class="card-item card-preview"
                                            ...
                                            <div class="card-content">
                                                ...
                                                <div
                                                    ...
                                                    @endif
                                                <!--there are the login-->
                                                    <a href="{{route('course.single',$course->slug)}}" class="theme-btn d-block addToCart-{{$course->id}}">@translate(Preview this course)</a>
                                    ...
                                    <div class="column-td-half">
                                        <div class="card-item card-preview"
                                            ...
                                            <div class="card-content">
                                                ...
                                                <div class="card-action">
                                                    <ul class="card-duration justify-content-between align-items-center">
                                                        <li>
                                                          <span class="meta__date">
                                                ...
                                                <div
                                                    ...
                                                    @endif
                                                <!--there are the login-->
                                                    <a href="{{route('course.single',$moreCourseItem->slug)}}" class="theme-btn d-block addToCart-{{$moreCourseItem->id}}">@translate(Preview this course)</a>
                                                </div><!-- end card-price-wrap -->
                                            </div><!-- end card-content -->
                                        </div>
                                    </div>
                                            ...
                                            @auth()
                                                @if(\Illuminate\Support\Facades\Auth::user()->user_type == 'Student')
                                                    <form action="{{ route('subscription.cart') }}" method="get">
                                                        ...
                                                        <!-- TODO 1 subscription for 2 course ? -->
                                                        @foreach (App\SubscriptionCourse::where('subscription_id', $s_course->subscription->subscription_id)->get() as $item)
                                                            <input type="hidden" name="course_id[]" value="{{ $item->course_id }}">
                                                        @endforeach
                                                        ...
                                                        @auth
                                                            ...
                                                            @endphp
                                                            @if (!$subscriptionEnrollment->exists())
                                                                <button type="submit" class="theme-btn mt-3">@translate(Subscribe now!)</button>
                                                            @elseif(!expire($subscriptionEnrollment->first()->subscription_id))
                                                                <button type="submit" class="theme-btn mt-3">@translate(Renew Subscription)</button>
                                                            @else
                                                                <button type="button" disabled class="theme-btn mt-3">@translate(Purchased)</button>
                                                            @endif
                                                        @endauth
                                                    </form>
                                                @else
