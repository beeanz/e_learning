...
class MessageController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($course as $c) {
            $course_id = array_merge($course_id, [$c->id]);
            $sub_us = array_merge($sub_us, [$c->subscription->subscription_id]);
        }
        $enrolls = Enrollment::whereIn('course_id', $course_id)->with('messages')->get();
        $subsEnrolls = SubscriptionEnrollment::whereIn('subscription_id', $sub_us)->with('messages')->get();
    ...
    public function send(Request $request)
    {
        ...
        if(!$e) {
            $e = SubscriptionEnrollment::where('id', $request->enroll_id)->with('enrollCourse')->first();
        }
