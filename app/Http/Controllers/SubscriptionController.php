...
class SubscriptionController extends Controller
{
     ...
     * PACKAGE COURSES
     */
    public function packageCourse($package, $subscription_id)
    {
        if (adminPower()) {
            $courses = SubscriptionCourse::where('subscription_duration', 'LIKE', '%' . $package . '%')->with('course')
                ->where('subscription_id', $subscription_id)->paginate(10);
            return view('subscription.subscription_coureses', compact('courses', 'package'));
        } else {
            $courses = SubscriptionCourse::where('subscription_duration', 'LIKE', '%' . $package . '%')->where('user_id', Auth::user()->id)->with('course')
                ->where('subscription_id', $subscription_id)->paginate(10);
            return view('subscription.subscription_coureses', compact('courses', 'package'));
        }
    ...
    public function courses()
    {
        ...
        $durations = Subscription::all()->unique('duration');
    ...
    public function cartList(Request $request)
    {
        ...
        //Get cards for Payme
        $my_cards = Card::where('user_id', auth()->id())->where('verify', 1)->get();

        $checkCart = SubscriptionEnrollment::where('subscription_id', $subscription_id)
        ...
        $subscriptionEnrollment->subscription_id = $subscription_id;
        $subscriptionEnrollment->course_id = reset($courses_id);
        ...
        return view('subscription.frontend.cart', compact('courses', 'subscription_price', 'subscription_package', 'subscription_id', 'my_cards'));
    }
    ...
    public function my_subscription_package_course($package, $subscription_id)
    {

        switch ($package) {
            case 'Monthly':

                $sub = SubscriptionEnrollment::where('subscription_package', $package)->where('subscription_id', $subscription_id)->where('end_at', '>', Carbon::now())->where('user_id', Auth::user()->id)->count();

                if ($sub > 0) {
                    $enrolls = SubscriptionCourse::where('subscription_duration', 'LIKE', '%' . $package . '%')->where('subscription_id', $subscription_id)->with('course')->paginate(10);
            ...
            case 'Free':
                ...
                $sub = SubscriptionEnrollment::where('subscription_package', $package)->where('subscription_id', $subscription_id)->where('user_id', Auth::user()->id)->count();
    ...
    public function subscription_lesson_details($slug)
    {
        ...
        $enroll = SubscriptionEnrollment::where('user_id', \Illuminate\Support\Facades\Auth::id())->where('subscription_id', $s_course->subscription->subscription_id)->get();
        if ($enroll->count() == 0) {
            return back();
        }
    ...
    public function changeEndAt(Request $request) {
        ...
        $subIdArr = $request->subs_id;
        $end_atArr = $request->end_at;

        $subId = reset($subIdArr);
        $end_at = reset($end_atArr);

        $end_at_frmt = Carbon::createFromFormat('d/m/Y', $end_at);

        $subscriptionEnrollment = SubscriptionEnrollment::findOrFail($subId);
        $update = $subscriptionEnrollment->update(array('end_at' => $end_at_frmt));
