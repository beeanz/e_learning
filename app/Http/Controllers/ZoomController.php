...
class ZoomController extends Controller
{
    ...
    public function show($meetingid){
        ...
        return view('zoom.show',compact('response', 'meeting'));
    }
    ...
    public function store(Request $request){
        ...
        $date = date_create_from_format('d/m/Y H:i A', $request->start_time);
        ...
        if(isset($request->recurring)){
            ...
            $start_time = date_format($date, 'Y-m-d\TH:i:s');
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => "{\"topic\":\"$request->topic\",\"type\":\"$type\",\"start_time\":\"$start_time\",\"duration\":\"$duration\",\"timezone\":\"$timezone\",\"password\":\"$request->password\",\"agenda\":\"$request->agenda\",\"settings\":{\"host_video\":\"$host_video\",\"participant_video\":\"$participant_video\",\"cn_meeting\":\"false\",\"in_meeting\":\"false\",\"join_before_host\":\"$join_before_host\",\"mute_upon_entry\":\"$mute_upon_entry\",\"watermark\":\"false\",\"use_pmi\":\"false\",\"approval_type\":\"2\",\"registration_type\":\"2\",\"audio\":\"both\",\"auto_recording\":\"none\",\"enforce_login\":\"false\",\"enforce_login_domains\":\"\",\"alternative_hosts\":\"\",\"global_dial_in_countries\":[\"\"],\"registrants_email_notification\":\"$registrants_email_notification\"}}",
            CURLOPT_HTTPHEADER => array(
                "authorization: Bearer $token",
        ...
        $created_meeting->start_time = $meeting_time;
        ...
        $created_meeting->created_at  = Carbon::now()->toDateTimeString();
        $created_meeting->updated_at  = Carbon::now()->toDateTimeString();
        $created_meeting->save();

        $emails = Enrollment::where('course_id', $course_id)->with('student_email')->with('course')->get();
        $emailsSubs = SubscriptionEnrollment::where('course_id', $course_id)->with('student_email')->with('course')->get();
        $course_name = SubscriptionEnrollment::where('course_id', $course_id)->with('course')->first();
        $emails = $emails->merge($emailsSubs);
