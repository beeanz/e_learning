...
class CourseController extends Controller
{
    ...
    */
    ...
    public function store(Request $request)
    {
        ...
        //summernote
        $courses->short_description = save64ToFile($request->short_description, 'course');
        $courses->big_description = save64ToFile($request->big_description,'course');
        ...
        //create one subscription for the course
        self::saveSubscription($request, $courses);
    ...
    public function destroy($course_id)
    {
        ...
        clearFilesFromDisk($course->short_description);
        clearFilesFromDisk($course->big_description);
        $course->delete();
    ...
    public function update(Request $request)
    {
        ...
        self::updateSubscription($request);
    ...
    private function saveSubscription(Request $request, $course)
    {
        // save subscription_course
        $subscription = new Subscription();
        $subscription->name = $request->slug;
        $subscription->description = json_encode(array($request->title));
        $subscription->price = $request->price;
        $subscription->instructor_payment = 0;
        ...
        // add course on subscription
        if ($subscription->duration != null) {

            $subs = array();


            foreach (array($request->duration) as $data) {
                $item = explode('-', $data);
                array_push($subs, $item[0]);
            }

            $subscription_courses = SubscriptionCourse::where('course_id', $course->id)->first();
            ...
            if ($subscription_courses != null) {
                $subscription_courses->update([
                    ...
                    'is_published' => 1,
                    'subscription_duration' => json_encode(array($request->title))
                ]);
            } else {
                $subscription_courses = new SubscriptionCourse();
                $subscription_courses->course_id = $course->id;
                $subscription_courses->user_id = $course->user_id;
                $subscription_courses->is_published = 1;
                $subscription_courses->subscription_id = $subscription->id;
    ...
    private function updateSubscription(Request $request)
    {
        $package = Subscription::where('name',$request->slug)->first();
        $package->name = $request->slug;
        $package->description = json_encode(array($request->title));
